    ...
    for priv in privlist:
        ...
        if privs == ["all"]:
            ...
            for prv in privs:
                ...
                if (
                    isinstance(prv, dict)
                    and isinstance(prv[key], dict)
                    and "grantable" in prv[key]
                    and prv[key]["grantable"]
                ):
                    prvcodes += "*"
...
def diff_privs(currobj, currlist, newobj, newlist, subobj=""):
    ...
    def rejoin(privdict, usr, grantor):
        return "%s=%s/%s" % (
            "" if usr == "PUBLIC" else usr,
            privdict[(usr, grantor)],
            grantor,
        )
    ...
    for (usr, gtor) in currprivs:
        if (usr, gtor) not in newprivs:
            stmts.append(
                add_revoke(currobj, rejoin(currprivs, usr, gtor), subobj)
            )
    for (usr, gtor) in newprivs:
        if (usr, gtor) not in currprivs:
            stmts.append(add_grant(newobj, rejoin(newprivs, usr, gtor), subobj))
        else:
            if sorted(currprivs[(usr, gtor)]) != sorted(newprivs[(usr, gtor)]):
                ...
                stmts.append(
                    add_grant(newobj, rejoin(newprivs, usr, gtor), subobj)
                )
    return stmts
