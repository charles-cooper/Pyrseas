...
CREATE_STOR_PARAMS = (
    CREATE_STMT + " WITH (fillfactor=90, autovacuum_enabled=false)"
)
...
class TableToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_table_comment_quotes(self):
        ...
        stmts = [
            CREATE_STMT,
            "COMMENT ON TABLE t1 IS " "'A \"special\" person''s table t1'",
        ]
    ...
    def test_map_column_comments(self):
        ...
        assert dbmap["schema sd"]["table t1"] == {
            "columns": [
                ...
                {"c2": {"type": "text", "description": "Test column c2 of t1"}},
            ]
        }
    ...
    def test_map_table_quoted(self):
        ...
        stmts = [
            ...
            'CREATE TABLE "a schema"."The.Table" ("column" SERIAL, ' "c2 TEXT)",
        ]
        dbmap = self.to_map(stmts)
            ...
            "columns": [
                {
                    "column": {
                        "type": "integer",
                        "not_null": True,
                        "default": 'nextval(\'"a schema"."The.Table_column_seq"\'::regclass)',
                    }
                },
                {"c2": {"type": "text"}},
            ]
    ...
    def test_map_table_sequence(self):
        ...
        stmts = [
            ...
            "ALTER SEQUENCE seq1 OWNED BY t2.c1",
...
class TableToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_table_quoted_idents(self):
        ...
        assert (
            fix_indent(sql[0]) == 'CREATE TABLE sd."order" ('
    ...
    def test_change_table_options(self):
        ...
        assert (
            fix_indent(sql[0]) == "ALTER TABLE sd.t1 SET (fillfactor=70),"
    ...
    def test_create_partitioned_tables(self):
        ...
        inmap["schema sd"].update(
            {
                ...
                "table t1a": {
                    "partition_bound_spec": spec1,
                    "partition_of": "t1",
                },
                "table t1b": {
                    "partition_bound_spec": spec2,
                    "partition_of": "t1",
                },
            }
        )
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[0]) == "CREATE TABLE sd.t1 (c1 date, "
...
class TableCommentToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_table_comment_quotes(self):
        ...
        inmap = self._tblmap()
        inmap["schema sd"]["table t1"][
            "description"
        ] = 'A "special" person\'s table t1'
        sql = self.to_sql(inmap, [CREATE_STMT])
        assert sql == [
            "COMMENT ON TABLE sd.t1 IS " "'A \"special\" person''s table t1'"
        ]
    ...
    def test_add_column_with_comment(self):
        ...
        inmap = self._tblmap()
        inmap["schema sd"]["table t1"]["columns"].append(
            {"c3": {"description": "Test column c3", "type": "integer"}}
        )
        sql = self.to_sql(inmap, [CREATE_STMT, COMMENT_STMT])
...
class TableInheritToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_table_inheritance(self):
        ...
        assert fix_indent(sql[0]) == CREATE_STMT
        assert (
            fix_indent(sql[1]) == "CREATE TABLE sd.t2 (c3 numeric) "
            "INHERITS (sd.t1)"
        )
    ...
    def test_table_inherit_delim(self):
        ...
        inmap["schema s-d"].update(
            {
                "table t-2": {
                    "columns": [
                        {"c-1": {"type": "integer", "inherited": True}},
                        {"c2": {"type": "text", "inherited": True}},
                        {"c3": {"type": "numeric"}},
                    ],
                    "inherits": ["t-1"],
        ...
        print(sql)
        assert (
            fix_indent(sql[0]) == 'CREATE TABLE "s-d"."t-1" '
            '("c-1" integer, c2 text)'
        )
        assert (
            fix_indent(sql[1]) == 'CREATE TABLE "s-d"."t-2" '
    ...
    def test_drop_inherited(self):
        ...
        stmts = [
            CREATE_STMT,
            "CREATE TABLE t2 (c3 numeric) INHERITS (t1)",
