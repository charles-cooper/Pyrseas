...
CREATE_STMT1 = (
    "CREATE CAST (smallint AS boolean) WITH FUNCTION " "sd.int2_bool(smallint)"
)
...
class CastToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_cast_comment(self):
        "Map a cast comment"
        dbmap = self.to_map(
            [CREATE_FUNC, CREATE_STMT1, COMMENT_STMT], superuser=True
        )
        assert (
            dbmap["cast (smallint as boolean)"]["description"] == "Test cast 1"
        )
...
class CastToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_cast_inout(self):
        ...
        inmap = self.std_map()
        inmap.update(
            {"cast (d1 as integer)": {"context": "implicit", "method": "inout"}}
        )
    ...
    def test_create_cast_schema(self):
        ...
        inmap.update(
            {
                "cast (integer as s1.d1)": {
                    "context": "assignment",
                    "method": "binary coercible",
                }
            }
        )
        ...
        sql = self.to_sql(inmap, stmts)
        assert (
            fix_indent(sql[0]) == "CREATE CAST (integer AS s1.d1) "
    ...
    def test_cast_function_view_depends(self):
        ...
        assert len(sql) == 3
        assert (
            fix_indent(sql[0]) == "CREATE VIEW sd.v1 AS SELECT t1.id FROM sd.t1"
        )
        assert (
            fix_indent(sql[1]) == "CREATE FUNCTION sd.v1_to_t1(sd.v1) "
            "RETURNS sd.t1 LANGUAGE plpgsql AS $_$\nDECLARE o sd.t1;\n"
            "BEGIN o:= ROW($1.id)::t1;\nRETURN o;\nEND$_$"
        )
        assert (
            fix_indent(sql[2]) == "CREATE CAST (sd.v1 AS sd.t1) WITH "
            "FUNCTION sd.v1_to_t1(sd.v1)"
        )
