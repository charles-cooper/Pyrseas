...
class ColumnToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_not_null(self):
        ...
        stmts = [
            "CREATE TABLE t1 (c1 INTEGER, c2 INTEGER NULL, "
            "c3 INTEGER NOT NULL)"
        ]
    ...
    def test_column_defaults(self):
        ...
        stmts = [
            "CREATE TABLE t1 (c1 INTEGER DEFAULT 12345, "
    ...
    def test_statistics(self):
        ...
        expmap = {
            "columns": [
                {"c1": {"type": "integer", "statistics": 100}},
                {"c2": {"type": "text"}},
            ]
        }
        assert dbmap["schema sd"]["table t1"] == expmap
...
class ColumnToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_table_with_defaults(self):
                        ...
                        "columns": [
                            {
                                "c1": {
                                    "type": "integer",
                                    "not_null": True,
                                    "default": "nextval('s1.t1_c1_seq'::regclass)",
                                }
                            },
                            {
                                "c2": {
        ...
        assert (
            fix_indent(sql[0]) == "CREATE SEQUENCE s1.t1_c1_seq "
            "START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1"
        )
        assert (
            fix_indent(sql[1]) == "CREATE TABLE s1.t1 (c1 integer "
    ...
    def test_alter_statistics(self):
        ...
        sql = self.to_sql(
            inmap,
            [CREATE_STMT1, "ALTER TABLE t1 ALTER c2 " "SET STATISTICS 1000"],
        )
        assert (
            fix_indent(sql[0])
