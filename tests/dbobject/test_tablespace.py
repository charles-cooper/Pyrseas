...
class ToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_primary_key(self):
        ...
        expmap = {
            ...
            "primary_key": {
                "t1_pkey": {"columns": ["c1"], "tablespace": "ts1"}
            },
        }
        assert dbmap["schema sd"]["table t1"] == expmap
    ...
    def test_map_index(self):
        ...
        dbmap = self.to_map(
            [
                ...
                "CREATE UNIQUE INDEX t1_idx ON t1 (c1) " "TABLESPACE ts1",
            ]
        )
        expmap = {
            "t1_idx": {"keys": ["c1"], "tablespace": "ts1", "unique": True}
        }
...
class ToSqlTestCase(InputMapToSqlTestCase):
        ...
        inmap["schema sd"].update(
            {
                "table t1": {
                    ...
                    "primary_key": {
                        "t1_pkey": {"columns": ["c1"], "tablespace": "ts2"}
                    },
                    "tablespace": "ts1",
                }
            }
    ...
    def test_create_index(self):
        ...
        sql = self.to_sql(inmap, [CREATE_TABLE])
        assert (
            fix_indent(sql[0]) == "CREATE UNIQUE INDEX t1_idx "
