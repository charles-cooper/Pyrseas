...
CREATE_FUNC_STMT = (
    "CREATE FUNCTION sd.f1() RETURNS event_trigger "
    "LANGUAGE plpgsql AS $_$%s$_$" % FUNC_SRC
)
CREATE_STMT = (
    "CREATE EVENT TRIGGER et1 ON ddl_command_end %s" "EXECUTE PROCEDURE sd.f1()"
)
...
class EventTriggerToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_event_trigger_filter(self):
        ...
        stmts = [
            CREATE_FUNC_STMT,
            CREATE_STMT % ("WHEN tag IN ('CREATE TABLE', 'CREATE VIEW') "),
        ]
        dbmap = self.to_map(stmts)
        assert dbmap["event trigger et1"] == {
            "enabled": True,
    ...
    def test_map_event_trigger_comment(self):
        ...
        dbmap = self.to_map(stmts)
        assert (
            dbmap["event trigger et1"]["description"]
...
class EventTriggerToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_event_trigger_func_schema(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE FUNCTION s1.f1() "
            "RETURNS event_trigger LANGUAGE plpgsql AS $_$%s$_$" % FUNC_SRC
        )
        assert (
            fix_indent(sql[1]) == "CREATE EVENT TRIGGER et1 "
