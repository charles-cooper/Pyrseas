...
CREATE_FUNC = (
    "CREATE FUNCTION f1() RETURNS text LANGUAGE sql IMMUTABLE AS "
    "$_$%s$_$" % SOURCE1
)
...
class PrivilegeToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_function(self):
        ...
        stmts = [
            CREATE_FUNC,
            "REVOKE ALL ON FUNCTION f1() FROM PUBLIC",
        ...
        expmap = {
            ...
            "privileges": [{self.db.user: ["execute"]}, {"user1": ["execute"]}],
        }
    ...
    def test_map_fd_wrapper(self):
        ...
        expmap = self.sort_privileges(
            {"privileges": [{self.db.user: ["usage"]}, {"PUBLIC": ["usage"]}]}
        )
    ...
    def test_map_server(self):
        ...
        dbmap = self.to_map(stmts, no_privs=False, superuser=True)
        expmap = {
            "privileges": [{self.db.user: ["usage"]}, {"user1": ["usage"]}]
        }
    ...
    def test_map_foreign_table(self):
        ...
        assert dbmap["schema sd"]["foreign table ft1"] == self.sort_privileges(
            expmap
        )
...
class PrivilegeToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_table(self):
        ...
        assert (
            sql[5] == "GRANT TRIGGER, REFERENCES ON TABLE sd.t1 "
    ...
    def test_create_column_grants(self):
        ...
        assert (
            sql[6] == "GRANT INSERT (c2), UPDATE (c2) ON TABLE sd.t1 "
            "TO user2"
        )
    ...
    def test_column_change_grants(self):
        ...
        assert len(sql) == 2
        assert (
            sql[0] == "GRANT INSERT (c1), UPDATE (c1) ON TABLE sd.t1 "
            "TO user2"
        )
        assert (
            sql[1] == "REVOKE INSERT (c2), UPDATE (c2) ON TABLE sd.t1 "
            "FROM user2"
        )
    ...
    def test_view_new_grant(self):
        ...
        sql = sorted(
            self.to_sql(
                inmap, ["CREATE VIEW v1 AS " "SELECT now()::date AS today"]
            )
        )
        assert len(sql) == 2
    ...
    def test_create_fd_wrapper(self):
        ...
        assert (
            sql[2] == "GRANT USAGE ON FOREIGN DATA WRAPPER fdw1 "
            "TO %s" % self.db.user
        )
    ...
    def test_fd_wrapper_new_grant(self):
        ...
        assert (
            sql[1] == "GRANT USAGE ON FOREIGN DATA WRAPPER fdw1 "
            "TO %s" % self.db.user
        )
    ...
    def test_create_server(self):
        ...
        assert (
            sql[2] == "GRANT USAGE ON FOREIGN SERVER fs1 TO %s" % self.db.user
        )
    ...
    def test_server_new_grant(self):
        ...
        sql = sorted(
            self.to_sql(inmap, [CREATE_FDW, CREATE_FS], superuser=True)
        )
        assert len(sql) == 2
        assert (
            sql[0] == "GRANT USAGE ON FOREIGN SERVER fs1 TO %s" % self.db.user
        )
