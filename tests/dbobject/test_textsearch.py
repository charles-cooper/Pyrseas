...
CREATE_TSD_STMT = (
    "CREATE TEXT SEARCH DICTIONARY sd.tsd1 (TEMPLATE = simple, "
    "stopwords = 'english')"
)
CREATE_TSP_STMT = (
    "CREATE TEXT SEARCH PARSER sd.tsp1 (START = prsd_start, "
    "GETTOKEN = prsd_nexttoken, END = prsd_end, LEXTYPES = prsd_lextype, "
    "HEADLINE = prsd_headline)"
)
CREATE_TST_STMT = (
    "CREATE TEXT SEARCH TEMPLATE sd.tst1 (INIT = dsimple_init, "
    "LEXIZE = dsimple_lexize)"
)
COMMENT_TSC_STMT = (
    "COMMENT ON TEXT SEARCH CONFIGURATION sd.tsc1 IS "
    "'Test configuration tsc1'"
)
COMMENT_TSD_STMT = (
    "COMMENT ON TEXT SEARCH DICTIONARY sd.tsd1 IS " "'Test dictionary tsd1'"
)
COMMENT_TSP_STMT = (
    "COMMENT ON TEXT SEARCH PARSER sd.tsp1 IS " "'Test parser tsp1'"
)
COMMENT_TST_STMT = (
    "COMMENT ON TEXT SEARCH TEMPLATE sd.tst1 IS " "'Test template tst1'"
)
...
class TextSearchConfigToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_ts_config_comment(self):
        ...
        dbmap = self.to_map(stmts)
        assert (
            dbmap["schema sd"]["text search configuration tsc1"]["description"]
            == "Test configuration tsc1"
        )
...
class TextSearchConfigToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_ts_config_in_schema(self):
        ...
        inmap["schema sd"].update(
            {"text search configuration tsc1": {"parser": "s1.tsp1"}}
        )
        sql = self.to_sql(inmap, ["CREATE SCHEMA s1"])
        assert (
            fix_indent(sql[0]) == "CREATE TEXT SEARCH PARSER s1.tsp1 "
            "(START = prsd_start, GETTOKEN = prsd_nexttoken, "
            "END = prsd_end, LEXTYPES = prsd_lextype)"
        )
        assert (
...
class TextSearchDictToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_bad_map_ts_dict(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"tsd1": {"template": "simple", "options": "stopwords = 'english'"}}
        )
        with pytest.raises(KeyError):
            self.to_sql(inmap)
...
class TextSearchParserToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_ts_parser_comment(self):
        ...
        dbmap = self.to_map(stmts)
        assert (
            dbmap["schema sd"]["text search parser tsp1"]["description"]
            == "Test parser tsp1"
        )
...
class TextSearchTemplateToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_bad_map_ts_template(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"tst1": {"init": "dsimple_init", "lexize": "dsimple_lexize"}}
        )
        with pytest.raises(KeyError):
            self.to_sql(inmap)
