...
CREATE_STMT = (
    "CREATE OPERATOR sd.+ (PROCEDURE = textcat, LEFTARG = text, "
    "RIGHTARG = text)"
)
...
class OperatorToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_operator_comment(self):
        ...
        assert (
            dbmap["schema sd"]["operator +(text, text)"]["description"]
            == "Test operator +"
        )
...
class OperatorToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_operator1(self):
        ...
        inmap["schema sd"].update(
            {"operator +(text, text)": {"procedure": "textcat"}}
        )
    ...
    def test_create_operator_rightarg(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"operator +(NONE, text)": {"procedure": "upper"}}
        )
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[0]) == "CREATE OPERATOR sd.+ ("
    ...
    def test_create_operator_in_schema(self):
        ...
        inmap = self.std_map()
        inmap.update(
            {"schema s1": {"operator +(text, text)": {"procedure": "textcat"}}}
        )
        sql = self.to_sql(inmap, ["CREATE SCHEMA s1"])
        assert (
            fix_indent(sql[0]) == "CREATE OPERATOR s1.+ "
    ...
    def test_change_operator_comment(self):
        ...
        sql = self.to_sql(inmap, stmts)
        assert sql == [
            "COMMENT ON OPERATOR sd.+(text, text) IS " "'Changed operator +'"
        ]
