...
class MatViewToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_view_comment(self):
        ...
        assert (
            dbmap["schema sd"]["materialized view mv1"]["description"]
            == "Test matview mv1"
        )

    def test_map_view_index(self):
        "Map a materialized view with an index"
        stmts = [CREATE_TABLE, CREATE_STMT, "CREATE INDEX idx1 ON mv1 (mc3)"]
        dbmap = self.to_map(stmts)
        expmap = {
            "columns": [
                {"c1": {"type": "integer"}},
...
class MatViewToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_view(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE TABLE sd.t1 (c1 integer, "
            "c2 text, c3 integer)"
        )
        assert (
            fix_indent(sql[1]) == "CREATE MATERIALIZED VIEW sd.mv1 AS "
