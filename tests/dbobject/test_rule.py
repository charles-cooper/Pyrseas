...
class RuleToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_rule_instead(self):
        ...
        stmts = [CREATE_TABLE_STMT, CREATE_STMT % ("UPDATE", "INSTEAD NOTHING")]
        dbmap = self.to_map(stmts)
        assert dbmap["schema sd"]["table t1"]["rules"] == {
    ...
    def test_map_rule_comment(self):
        ...
        stmts = [
            CREATE_TABLE_STMT,
            CREATE_STMT % ("INSERT", "NOTHING"),
            COMMENT_STMT,
        ]
        dbmap = self.to_map(stmts)
        assert (
            dbmap["schema sd"]["table t1"]["rules"]["r1"]["description"]
            == "Test rule r1"
        )
...
class RuleToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_rule_instead(self):
        ...
        assert fix_indent(sql[0]) == CREATE_TABLE_STMT
        assert (
            fix_indent(sql[1]) == "CREATE RULE r1 AS ON UPDATE TO sd.t1 "
            "DO INSTEAD NOTHING"
        )
    ...
    def test_create_rule_conditional(self):
        ...
        assert fix_indent(sql[0]) == CREATE_TABLE_STMT
        assert (
            fix_indent(sql[1]) == "CREATE RULE r1 AS ON DELETE TO sd.t1 "
    ...
    def test_create_rule_multi_actions(self):
        ...
        assert fix_indent(sql[0]) == CREATE_TABLE_STMT
        assert (
            fix_indent(sql[1]) == "CREATE RULE r1 AS ON UPDATE "
    ...
    def test_create_rule_in_schema(self):
        ...
        assert (
            fix_indent(sql[1]) == "CREATE RULE r1 AS ON INSERT TO s1.t1 "
            "DO NOTHING"
        )
