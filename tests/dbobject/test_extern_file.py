...
class ExternalFilenameMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_casts(self):
        ...
        self.to_map(
            [
                "CREATE FUNCTION int2_bool(smallint) RETURNS boolean "
        ...
        expmap = {
            "cast (smallint as boolean)": {
                "function": "sd.int2_bool(smallint)",
                "context": "explicit",
                "method": "function",
            },
            "cast (sd.d1 as integer)": {
                "context": "implicit",
                "method": "inout",
    ...
    def test_map_extension(self):
        ...
        if self.db.version < 90300:
            ...
            TRGM_VERS = "1.3"
        self.to_map(
            ["CREATE EXTENSION pg_trgm"], superuser=True, multiple_files=True
        )
        expmap = {
            "extension plpgsql": {
                "schema": "pg_catalog",
    ...
    def test_map_fd_wrappers(self):
        ...
        self.to_map(
            [
                CREATE_FDW + "fdw1",
                CREATE_FDW + "fdw2 " "OPTIONS (debug 'true')",
            ],
            superuser=True,
            multiple_files=True,
        )
    ...
    def test_collations(self):
        ...
        self.to_map(
            [
                "CREATE COLLATION coll1 (LC_COLLATE = '%s', "
    ...
    def test_map_conversion(self):
        ...
        self.to_map(
            [
                "CREATE CONVERSION conv1 FOR 'LATIN1' TO 'UTF8' "
    ...
    def test_map_functions_merged(self):
        ...
        self.to_map(
            [
                "CREATE FUNCTION f3(integer) RETURNS text LANGUAGE sql "
        ...
        assert self.yaml_load("function.f3.yaml", "schema.sd") == {
            "function f3(integer)": expmap,
            "function f3(real)": expmap,
        }
    ...
    def test_map_operator(self):
        ...
        self.to_map(
            [
                "CREATE OPERATOR < (PROCEDURE = int4lt, LEFTARG = int, "
                "RIGHTARG = int)",
                "CREATE OPERATOR = (PROCEDURE = "
                "int4eq, LEFTARG = int, RIGHTARG = int)",
                "CREATE "
        ...
        oprmap = {
            "operator <(integer, integer)": {"procedure": "int4lt"},
            "operator =(integer, integer)": {"procedure": "int4eq"},
            "operator >(integer, integer)": {"procedure": "int4gt"},
        }
        opcmap = {
            "operator class oc1 using btree": {
                "type": "integer",
                "operators": {
                    1: "sd.<(integer,integer)",
                    3: "sd.=(integer,integer)",
    ...
    def test_map_tables_merged(self):
        ...
        self.to_map(
            [
                "CREATE TABLE account_transfers_with_extra_padding_1 "
                "(c1 integer, c2 text)",
        ...
        expmap = {
            "table account_transfers_with_extra_padding_1": {
                "columns": [
                    {"c1": {"type": "integer"}},
                    {"c2": {"type": "text"}},
                ]
            },
            "table account_transfers_with_extra_padding_2": {
                "columns": [
                    {"c1": {"type": "integer"}},
                    {"c2": {"type": "text"}},
                ]
        ...
        assert (
            self.yaml_load(
                "table.account_transfers_with_extra_pad.yaml", "schema.sd"
            )
            == expmap
        )
    ...
    def test_map_textsearch(self):
        ...
        assert self.yaml_load(
            "text_search_configuration.yaml", "schema.sd"
        ) == {
    ...
    def test_map_drop_table(self):
        ...
        for tbl in ["t1", "t2", "t3"]:
            assert (
                self.yaml_load("table.%s.yaml" % tbl, "schema.sd")[
                    "table %s" % tbl
                ]
                == expmap
            )
...
class ExternalFilenameTestCase(PyrseasTestCase):
    def test_function(self):
        "Map a function"
        obj = Function(
            "Weird/Or-what?", "sd", "", None, [], None, None, None, None
        )
