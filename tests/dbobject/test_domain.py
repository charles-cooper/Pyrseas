...
CREATEFUNC_STMT = (
    "CREATE FUNCTION sd.dc1(integer) RETURNS bool LANGUAGE sql "
...
class DomainToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_domain_depend_function(self):
        ...
        dbmap = self.to_map(
            [CREATEFUNC_STMT, CREATE_STMT + " CHECK (dc1(VALUE))"]
        )
        expmap = {
            "type": "integer",
            "check_constraints": {
...
class DomainToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_domain_default(self):
        ...
        inmap["schema sd"].update(
            {"domain d1": {"type": "integer", "not_null": True, "default": 0}}
        )
        sql = self.to_sql(inmap)
    ...
    def test_create_domain_check(self):
        ...
        assert (
            fix_indent(sql[1])
            == "ALTER DOMAIN sd.d1 ADD CONSTRAINT "
    ...
    def test_rename_domain(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"domain d2": {"oldname": "d1", "type": "integer"}}
        )
        sql = self.to_sql(inmap, [CREATE_STMT])
