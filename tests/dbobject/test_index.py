...
class IndexToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_index_2(self):
        ...
        expmap = {
            ...
            "indexes": {"t1_idx": {"keys": ["c1", "c2"], "unique": True}},
        }
        assert dbmap["schema sd"]["table t1"] == expmap
    ...
    def test_index_3(self):
        ...
        expmap = {
            ...
            "indexes": {
                "t1_idx_1": {"keys": ["c1", "c2"], "unique": True},
                "t1_idx_2": {"keys": ["c3"], "access_method": "gin"},
            },
        }
        assert dbmap["schema sd"]["table t1"] == expmap
    ...
    def test_map_index_partial(self):
        ...
        dbmap = self.to_map(
            [CREATE_TABLE_STMT, "CREATE INDEX t1_idx ON t1 (c2) WHERE c1 > 42"]
        )
        expmap = {
            "columns": [{"c1": {"type": "integer"}}, {"c2": {"type": "text"}}],
            "indexes": {"t1_idx": {"keys": ["c2"], "predicate": "(c1 > 42)"}},
        }
        assert dbmap["schema sd"]["table t1"] == expmap
    ...
    def test_index_function_simple(self):
        ...
        expmap = {
            "columns": [{"c1": {"type": "integer"}}, {"c2": {"type": "text"}}],
            "indexes": {
                "t1_idx": {"keys": [{"lower(c2)": {"type": "expression"}}]}
            },
        }
        assert dbmap["schema sd"]["table t1"] == expmap
        ...
        expmap = {
            ...
            "indexes": {
                "t1_idx1": {
                    "keys": [
                        {
                            '"substring"(c2, "position"(c2, \'_begin\''
                ...
                "t1_idx2": {
                    "keys": [
                        {
                            "date_part('month'::text, c3)": {
                                "type": "expression"
                            }
                        },
                        {"date_part('day'::text, c3)": {"type": "expression"}},
                    ]
                },
            },
        }
    ...
    def test_index_col_opts(self):
        ...
        stmts = [
            ...
            "CREATE INDEX t1_idx ON t1 (c1 cidr_ops NULLS FIRST, " "c2 DESC)",
        ]
        dbmap = self.to_map(stmts)
        expmap = {
    ...
    def test_map_index_cluster(self):
        ...
        dbmap = self.to_map(
            [CREATE_TABLE_STMT, CREATE_STMT, "CLUSTER t1 USING t1_idx"]
        )
        expmap = {
            "columns": [{"c1": {"type": "integer"}}, {"c2": {"type": "text"}}],
    ...
    def test_map_index_comment(self):
        ...
        assert (
            dbmap["schema sd"]["table t1"]["indexes"]["t1_idx"]["description"]
            == "Test index t1_idx"
        )
    ...
    def test_map_multicol_index_with_exprs(self):
        ...
        dbmap = self.to_map(
            [
                "CREATE TABLE holiday (id serial PRIMARY KEY,"
...
class IndexToSqlTestCase(InputMapToSqlTestCase):
        ...
        inmap["schema sd"].update(
            {
                "table t1": {
                    ...
                    "indexes": {
                        "t1_idx": {"keys": ["c2"], "predicate": "(c1 > 42)"}
                    },
                }
            }
        )
    ...
    def test_create_index_col_opts(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            sql[1] == "CREATE INDEX t1_idx ON sd.t1 "
    ...
    def test_create_index_mixed(self):
        ...
        assert (
            sql[0] == "CREATE INDEX t1_idx ON sd.t1 ("
            "btrim(c3, 'x'::text) NULLS FIRST, c1, lower(c2) DESC)"
        )
        assert (
            sql[1] == "CREATE INDEX t1_idx2 ON sd.t1 ("
