...
CREATE_STMT_FULL = (
    "CREATE SEQUENCE sd.seq1 %sSTART WITH 1 INCREMENT BY 1 "
...
class SequenceToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_sequence_comment(self):
        ...
        assert (
            dbmap["schema sd"]["sequence seq1"]["description"]
            == "Test sequence seq1"
        )
...
class SequenceToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_sequence_in_schema(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE SEQUENCE s1.seq1 START WITH 1 "
    ...
    def test_no_drop_owned_sequence(self):
        ...
        sql = self.to_sql(
            self.std_map(), ["CREATE TABLE t1 (c1 SERIAL, c2 text)"]
        )
    ...
    def test_change_sequence(self):
        ...
        sql = self.to_sql(inmap, [CREATE_STMT])
        assert (
            fix_indent(sql[0]) == "ALTER SEQUENCE sd.seq1 START WITH 5 "
...
class SequenceUndoSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_undo_change_sequence(self):
        ...
        assert (
            fix_indent(sql[0]) == "ALTER SEQUENCE sd.seq1 START WITH 1 "
            "INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1"
        )
