...
class CollationToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_collation_comment(self):
        ...
        assert (
            dbmap["schema sd"]["collation c1"]["description"]
            == "Test collation c1"
        )
...
class CollationToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_collation1(self):
        ...
        inmap["schema sd"].update(
            {"collation c1": {"lc_collate": COLL, "lc_ctype": COLL}}
        )
    ...
    def test_create_collation_schema(self):
        ...
        assert fix_indent(
            sql[0]
        ) == "CREATE COLLATION s1.c1 (" "LC_COLLATE = '%s', LC_CTYPE = '%s')" % (
            COLL,
            COLL,
        )
    ...
    def test_bad_collation_map(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"c1": {"lc_collate": COLL, "lc_ctype": COLL}}
        )
    ...
    def test_create_table_column_collation(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[0]) == "CREATE TABLE sd.t1 ("
    ...
    def test_create_index_collation(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE INDEX t1_idx ON sd.t1 (c2 COLLATE c1)"
        )
    ...
    def test_create_type_attribute_collation(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[0]) == "CREATE TYPE sd.t1 AS (x integer, "
            'y text COLLATE "c1")'
        )
