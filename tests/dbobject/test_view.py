...
class ViewToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_view_comment(self):
        ...
        assert dbmap["schema sd"]["view v1"]["description"] == "Test view v1"
...
class ViewToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_view(self):
        ...
        inmap["schema sd"].update(
            {"view v1": {"definition": "SELECT c1, c3 * 2 AS c2 FROM t1"}}
        )
        sql = self.to_sql(inmap)
    ...
    def test_rename_view(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"view v2": {"oldname": "v1", "definition": VIEW_DEFN}}
        )
    ...
    def test_bad_rename_view(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"view v2": {"oldname": "v3", "definition": VIEW_DEFN}}
        )
        with pytest.raises(KeyError):
    ...
    def test_change_view_defn(self):
        ...
        sql = self.to_sql(inmap, [CREATE_STMT])
        assert (
            fix_indent(sql[0]) == "CREATE OR REPLACE VIEW sd.v1 AS "
    ...
    def test_view_depend_pk(self):
        ...
        assert (
            fix_indent(sql[2]) == "CREATE VIEW sd.v1 AS SELECT t1.c1, "
