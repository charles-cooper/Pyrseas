...
class StaticTableToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_copy_static_table(self):
        ...
        recs = []
        with open(
            os.path.join(self.cfg["files"]["data_path"], "schema.sd", FILE_PATH)
        ) as f:
            for line in f:
                (c1, c2) = line.split(",")
    ...
    def test_copy_static_table_pk(self):
        ...
        self.db.execute(
            "CREATE TABLE t1 (c1 integer, c2 char(3), c3 text,"
            "PRIMARY KEY (c2, c1))"
        )
...
class StaticTableToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_load_static_table(self):
        ...
        copy_stmt = (
            ...
            " from '",
            os.path.join(
                self.cfg["files"]["data_path"], "schema.sd", FILE_PATH
            ),
            "' csv",
        )
    ...
    def test_load_static_table_fk(self):
        ...
        assert (
            sql[3] == "ALTER TABLE sd.t2 ADD CONSTRAINT t2_c2_fkey "
