...
FUNC_INSTEAD_SRC = (
    "BEGIN INSERT INTO t1 VALUES (NEW.c1, NEW.c2, now()); " "RETURN NULL; END"
)
CREATE_TABLE_STMT = "CREATE TABLE sd.t1 (c1 integer, c2 text, " "c3 date)"
CREATE_TABLE_STMT2 = (
    "CREATE TABLE t1 (c1 integer, c2 text, " "c3 text, tsidx tsvector)"
)
CREATE_FUNC_STMT = (
    "CREATE FUNCTION sd.f1() RETURNS trigger LANGUAGE plpgsql"
    " AS $_$%s$_$" % FUNC_SRC
)
CREATE_STMT = (
...
class TriggerToMapTestCase(DatabaseToMapTestCase):
        ...
        assert dbmap["schema sd"]["table t1"]["triggers"] == {
            "tr1": {
                ...
                "procedure": {
                    ...
                    "arguments": "'tsidx', 'pg_catalog.english', 'c2'",
                },
            }
        }
    ...
    def test_map_trigger_comment(self):
        ...
        stmts = [CREATE_TABLE_STMT, CREATE_FUNC_STMT, CREATE_STMT, COMMENT_STMT]
        dbmap = self.to_map(stmts)
        assert (
            dbmap["schema sd"]["table t1"]["triggers"]["tr1"]["description"]
            == "Test trigger tr1"
        )
...
class TriggerToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_trigger2(self):
        ...
        assert fix_indent(sql[crt1]) == CREATE_FUNC_STMT
        assert (
            fix_indent(sql[2]) == "CREATE TRIGGER tr1 AFTER DELETE OR "
    ...
    def test_create_trigger_update_cols(self):
        ...
        assert fix_indent(sql[crt1]) == CREATE_FUNC_STMT
        assert (
            fix_indent(sql[2]) == "CREATE TRIGGER tr1 BEFORE INSERT OR "
    ...
    def test_create_trigger_conditional(self):
        ...
        assert fix_indent(sql[crt1]) == CREATE_FUNC_STMT
        assert (
            fix_indent(sql[2]) == "CREATE TRIGGER tr1 BEFORE UPDATE "
    ...
    def test_create_trigger_instead(self):
        ...
        assert (
            fix_indent(sql[cr1]) == "CREATE VIEW sd.v1 AS SELECT c1, c2 FROM t1"
        )
        assert (
            fix_indent(sql[cr2]) == "CREATE FUNCTION sd.f1() RETURNS "
            "trigger LANGUAGE plpgsql AS $_$%s$_$" % FUNC_INSTEAD_SRC
        )
        assert (
            fix_indent(sql[3]) == "CREATE TRIGGER tr1 INSTEAD OF INSERT "
    ...
    def test_add_tsvector_trigger(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE TRIGGER t1_tsidx_update BEFORE"
    ...
    def test_change_tsvector_trigger(self):
                ...
                "table t1": {
                    ...
                    "triggers": {
                        "t1_tsidx_update": {
                            ...
                            "procedure": {
                                "name": "tsvector_update_trigger",
                                "arguments": "'tsidx', 'pg_catalog.english', 'c2', 'c3'",
                            },
                        }
                    },
                }
        ...
        assert (
            fix_indent(sql[1]) == "CREATE TRIGGER t1_tsidx_update BEFORE"
            " INSERT OR UPDATE ON sd.t1 FOR EACH ROW EXECUTE PROCEDURE "
...
class ConstraintTriggerToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_trigger(self):
        ...
        assert fix_indent(sql[crt1]) == CREATE_FUNC_STMT
        assert (
            fix_indent(sql[2]) == "CREATE CONSTRAINT TRIGGER tr1 AFTER "
    ...
    def test_create_trigger_deferrable(self):
        ...
        assert fix_indent(sql[crt1]) == CREATE_FUNC_STMT
        assert (
            fix_indent(sql[2]) == "CREATE CONSTRAINT TRIGGER tr1 "
