...
CREATE_STMT1 = (
    "CREATE FUNCTION sd.f1() RETURNS text LANGUAGE sql IMMUTABLE "
    "AS $_$%s$_$" % SOURCE1
)

SOURCE2 = "SELECT GREATEST($1, $2)"
CREATE_STMT2 = (
    "CREATE FUNCTION sd.f1(integer, integer) RETURNS integer "
    "LANGUAGE sql IMMUTABLE AS $_$%s$_$" % SOURCE2
)
COMMENT_STMT = (
    "COMMENT ON FUNCTION sd.f1(integer, integer) IS " "'Test function f1'"
)

SOURCE3 = "SELECT * FROM generate_series($1, $2)"
CREATE_STMT3 = (
    "CREATE FUNCTION f2(integer, integer) RETURNS SETOF integer "
    "ROWS 20 LANGUAGE sql IMMUTABLE AS $_$%s$_$" % SOURCE3
)

SOURCE4 = "SELECT $1 + $2"
CREATE_STMT4 = (
    "CREATE FUNCTION fadd(integer, integer) RETURNS integer "
    "LANGUAGE sql IMMUTABLE AS $_$%s$_$" % SOURCE4
)

SOURCE5 = "SELECT $1 - $2"
CREATE_STMT5 = (
    "CREATE FUNCTION fsub(integer, integer) RETURNS integer "
...
class FunctionToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_function_config(self):
        ...
        stmts = [
            "CREATE FUNCTION f1() RETURNS date LANGUAGE sql SET "
    ...
    def test_map_function_comment(self):
        ...
        assert (
            dbmap["schema sd"]["function f1(integer, integer)"]["description"]
            == "Test function f1"
        )
    ...
    def test_map_function_rows(self):
        ...
        assert dbmap["schema sd"]["function f2(integer, integer)"]["rows"] == 20
    ...
    def test_map_function_leakproof(self):
        ...
        assert dbmap["schema sd"]["function fadd(integer, integer)"] == expmap
...
class FunctionToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_function_with_args(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[1]) == "CREATE FUNCTION sd.f1(integer, integer)"
    ...
    def test_create_setof_row_function(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[2]) == "CREATE FUNCTION sd.f1() RETURNS "
    ...
    def test_create_setof_row_function_rows(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[2]) == "CREATE FUNCTION sd.f1() RETURNS SETOF "
    ...
    def test_create_security_definer_function(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[1]) == "CREATE FUNCTION sd.f1() RETURNS text "
    ...
    def test_create_c_lang_function(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[0]) == "CREATE FUNCTION sd.autoinc() "
    ...
    def test_create_function_config(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[1]) == "CREATE FUNCTION sd.f1() RETURNS date "
    ...
    def test_create_function_in_schema(self):
        ...
        assert (
            fix_indent(sql[1]) == "CREATE FUNCTION s1.f1() RETURNS text "
    ...
    def test_bad_function_map(self):
        ...
        inmap = self.std_map()
        inmap["schema sd"].update(
            {"f1()": {"language": "sql", "returns": "text", "source": SOURCE1}}
        )
    ...
    def test_change_function_defn(self):
        ...
        assert (
            fix_indent(sql[1]) == "CREATE OR REPLACE FUNCTION sd.f1() "
    ...
    def test_change_function_comment(self):
        ...
        sql = self.to_sql(inmap, stmts)
        assert sql == [
            "COMMENT ON FUNCTION sd.f1(integer, integer) IS "
            "'Changed function f1'"
        ]
...
class AggregateToMapTestCase(DatabaseToMapTestCase):
    ...
    def test_map_aggregate_sortop(self):
        ...
        dbmap = self.to_map(stmts)
        expmap = {"sfunc": "f1", "stype": "integer", "sortop": "pg_catalog.>"}
        assert dbmap["schema sd"]["aggregate a1(integer)"] == expmap
    ...
    def test_map_ordered_set_aggregate(self):
        ...
        stmts = [
            CREATE_STMT2,
            "CREATE AGGREGATE a1 (integer ORDER BY "
            "integer) (sfunc = f1, stype = integer)",
        ]
        dbmap = self.to_map(stmts)
        expmap = {"sfunc": "f1", "stype": "integer", "kind": "ordered"}
        assert (
            dbmap["schema sd"]["aggregate a1(integer ORDER BY integer)"]
            == expmap
        )
...
class AggregateToSqlTestCase(InputMapToSqlTestCase):
    ...
    def test_create_aggregate_simple(self):
        ...
        inmap["schema sd"].update(
            {"aggregate a1(integer)": {"sfunc": "f1", "stype": "integer"}}
        )
        sql = self.to_sql(inmap)
        assert fix_indent(sql[1]) == CREATE_STMT2
        assert (
            fix_indent(sql[2]) == "CREATE AGGREGATE sd.a1(integer) "
    ...
    def test_create_aggregate_sortop(self):
        ...
        sql = self.to_sql(inmap)
        assert (
            fix_indent(sql[2]) == "CREATE AGGREGATE sd.a1(float) "
    ...
    def test_create_aggregate_init_final(self):
        ...
        assert (
            fix_indent(funcs[1]) == "CREATE FUNCTION sd.f2(integer) "
            "RETURNS double precision LANGUAGE sql IMMUTABLE "
            "AS $_$SELECT $1::float$_$"
        )
        assert (
            fix_indent(sql[3]) == "CREATE AGGREGATE sd.a1(integer) "
    ...
    def test_drop_aggregate(self):
        ...
        stmts = [
            CREATE_STMT2,
            "CREATE AGGREGATE agg1 (integer) " "(SFUNC = f1, STYPE = integer)",
        ]
    ...
    def test_create_moving_aggregate(self):
        ...
        assert (
            fix_indent(sql[0]) == "CREATE AGGREGATE sd.a1(integer) ("
