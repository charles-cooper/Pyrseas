...
class AuditColumnsTestCase(AugmentToMapTestCase):
    ...
    def test_predef_column(self):
        ...
        augmap = {
            "schema sd": {"table t1": {"audit_columns": "created_date_only"}}
        }
    ...
    def test_unknown_table(self):
        ...
        augmap = {
            "schema sd": {"table t2": {"audit_columns": "created_date_only"}}
        }
    ...
    def test_bad_audit_spec(self):
        ...
        augmap = {
            "schema sd": {"table t1": {"audit_column": "created_date_only"}}
        }
        with pytest.raises(KeyError):
            self.to_map([CREATE_STMT], augmap)
    ...
    def test_unknown_audit_spec(self):
        ...
        augmap = {"schema sd": {"table t1": {"audit_columns": "created_date"}}}
        with pytest.raises(KeyError):
            self.to_map([CREATE_STMT], augmap)
    ...
    def test_new_column(self):
        ...
        augmap = {
            "augmenter": {
                ...
                "audit_columns": {
                    "modified_date_only": {"columns": ["modified_date"]}
                },
            },
            "schema sd": {"table t1": {"audit_columns": "modified_date_only"}},
        }
        dbmap = self.to_map([CREATE_STMT], augmap)
        expmap = {
            "columns": [
    ...
    def test_rename_column(self):
        ...
        augmap = {
            "augmenter": {
                "columns": {"modified_timestamp": {"name": "updated"}}
            },
            "schema sd": {"table t1": {"audit_columns": "modified_only"}},
        }
        dbmap = self.to_map([CREATE_STMT], augmap)
        colmap = {
            "columns": [
            ...
            "triggers": {
                "t1_20_audit_modified_only": {
                    "events": ["insert", "update"],
                    "level": "row",
                    "procedure": "sd.audit_modified",
                    "timing": "before",
                }
            },
        }
        funcmap = {
            ...
            "description": "Provides modified_timestamp values for audit columns.",
            "source": FUNC_SRC2,
        }
    ...
    def test_change_column_type(self):
        ...
        augmap = {
            "augmenter": {"columns": {"created_date": {"type": "text"}}},
            "schema sd": {"table t1": {"audit_columns": "created_date_only"}},
        }
        dbmap = self.to_map([CREATE_STMT], augmap)
        expmap = {
    ...
    def test_columns_with_trigger(self):
        ...
        assert expmap == dbmap["schema sd"]["table t1"]
        assert (
            dbmap["schema sd"]["function audit_default()"]["returns"]
            == "trigger"
        )
        assert (
            dbmap["schema sd"]["function audit_default()"]["source"]
            == FUNC_SRC1
        )

    def test_nondefault_schema_with_trigger(self):
        "Add predefined audit columns with trigger in a non-default schema"
        stmts = ["CREATE SCHEMA s1", "CREATE TABLE s1.t1 (c1 integer, c2 text)"]
        ...
        assert (
            dbmap["schema s1"]["function audit_default()"]["returns"]
            == "trigger"
        )
        assert (
            dbmap["schema s1"]["function audit_default()"]["source"]
            == FUNC_SRC1
        )
    ...
    def test_change_existing_columns(self):
        ...
        stmts = [
            CREATE_STMT,
            "ALTER TABLE t1 ADD modified_by_user text ",
            "ALTER TABLE t1 ADD modified_timestamp "
            "timestamp with time zone NOT NULL",
        ]
        augmap = {"schema sd": {"table t1": {"audit_columns": "default"}}}
    ...
    def test_custom_function_template(self):
        ...
        augmap = {
            ...
            "schema sd": {"table t1": {"audit_columns": "custom"}},
        }
        dbmap = self.to_map([CREATE_STMT], augmap)
        expmap = {
            "columns": [
        ...
        assert expmap == dbmap["schema sd"]["table t1"]
        assert (
            dbmap["schema sd"]["function custom_audit()"]["returns"]
            == "trigger"
        )
        assert dbmap["schema sd"]["function custom_audit()"]["source"] == source
